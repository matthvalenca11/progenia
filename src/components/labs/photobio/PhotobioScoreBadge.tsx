/**
 * Badge de pontuação do modo guiado (0–100) — PBM
 */

import { Trophy, Star, TrendingUp, AlertCircle } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import { photobioScoreTone, PHOTOBIO_SCORE_TONE_LABELS } from "@/lib/photobioScoring";

interface PhotobioScoreBadgeProps {
  score: number;
  compact?: boolean;
  showLabel?: boolean;
  className?: string;
...
const TONE_STYLES = {
  ...
  fair: "border-rose-500/40 bg-rose-500/10 text-rose-700 dark:text-rose-300",
  low: "border-red-500/40 bg-red-500/10 text-red-700 dark:text-red-300",
} as const;

function ToneIcon({ tone }: { tone: ReturnType<typeof photobioScoreTone> }) {
...
export function PhotobioScoreBadge({
  score,
  compact = false,
  showLabel = true,
  className,
}: PhotobioScoreBadgeProps) {
  const tone = photobioScoreTone(score);
  ...
  return (
    <div className={cn("flex items-center gap-2 rounded-xl border px-3 py-2", TONE_STYLES[tone], className)}>
      <ToneIcon tone={tone} />
      <div className="min-w-0">
        ...
        {showLabel && (
          <p className="mt-0.5 text-[10px] font-medium uppercase tracking-wide opacity-80">
            {PHOTOBIO_SCORE_TONE_LABELS[tone]}
          </p>
        )}
      </div>
    </div>
