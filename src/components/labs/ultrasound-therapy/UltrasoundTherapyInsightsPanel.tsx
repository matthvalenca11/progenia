...
import { Thermometer, AlertTriangle, Zap, Target, Droplets } from "lucide-react";
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {simulationResult.risk !== "low" && (
          <div className="p-2.5 bg-muted/50 rounded-lg border border-border">
            <div className="text-[11px] font-medium text-foreground mb-1.5">Recomendações</div>
            <div className="space-y-1">
              ...
              {config.coupling === "poor" && (
                <p className="text-[10px] text-muted-foreground">• Melhore o acoplamento para reduzir risco superficial</p>
              )}
              {simulationResult.cumulativeDose > 120 && (
                <p className="text-[10px] text-muted-foreground">• Dose térmica acumulada muito alta - interrompa o tratamento</p>
              )}
              {simulationResult.periostealRisk > 0.5 && (
                <p className="text-[10px] text-muted-foreground">• Reduza intensidade ou mude posição - risco periosteal elevado</p>
              )}
            </div>
          </div>
        )}
